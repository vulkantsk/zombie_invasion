function Spawn( entityKeyValues )
    ...
    hDecayAbility = thisEntity:FindAbilityByName( "undying_decay_boss_wave_2" )
 
	thisEntity:SetContextThink( "UndThink", UndThink, 1 )
end
...
function UndThink()
	...
	if   hDecayAbility:IsFullyCastable() then
		return  CastDecay( enemies[ RandomInt( 1, #enemies ) ] )
	end
...
function CastDecay( enemy )
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_POSITION,
					Position = enemy:GetOrigin(),
		AbilityIndex = hDecayAbility:entindex(),
	})
