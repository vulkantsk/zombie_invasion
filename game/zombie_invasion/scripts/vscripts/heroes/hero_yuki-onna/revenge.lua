...
function modifier_revenge_buff:OnIntervalThink()
	local base_damage = self:GetAbility():GetSpecialValueFor("base_damage")
 	local damage = self:GetAbility():GetSpecialValueFor("damage_per_int") * self:GetCaster():GetIntellect(true) / (1 + self:GetCaster():GetSpellAmplification(false)) + base_damage 
