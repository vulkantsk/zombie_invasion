...
LinkLuaModifier("modifier_item_imba_iron_talon", "items/other/iron_talon_custom", LUA_MODIFIER_MOTION_NONE)
...
function item_imba_iron_talon:CastFilterResultTarget(hTarget)
	...
	if hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and not hTarget:IsBoss() then
		...
		not hTarget:IsRoshan() then
			return UF_SUCCESS
		elseif hTarget:IsBoss() and not (string.find(hTarget:GetName(), "npc_dota_ward_base") or string.find(hTarget:GetName(), "npc_dota_techies_mines")) then
