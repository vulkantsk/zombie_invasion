function Spawn( entityKeyValues )
    ...
    hSlowAbility = thisEntity:FindAbilityByName( "boss_zombie_open_wounds" )

	thisEntity:SetContextThink( "ZombieHalfThink", ZombieHalfThink, 1 )
end

 


function ZombieHalfThink()
	...
	if   hSlowAbility:IsFullyCastable() then
		return  CastSlow( enemies[ 1 ] )
	end
...
function CastSlow( enemy )
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_TARGET,
					TargetIndex = enemy:entindex(),
		AbilityIndex = hSlowAbility:entindex(),
	})
