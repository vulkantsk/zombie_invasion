...
PHASE_PHYSICAL = 0   --身体阶段
PHASE_SANDSTORM = 1   --沙尘暴阶段
PHASE_EPICENTER = 2   --地震阶段

TRIGGER_PHASE_CD = 12
PHASE_DURATION = 15   --肉搏持续时间
BURROW_DURATION = 15  --挖地道持续时间
MIN_BURROWSTRIKES = 2  --最小挖洞时间
MAX_BURROWSTRIKES = 5  --最大挖洞时间
...
function Spawn( entityKeyValues )
	if IsServer() then
		...
		BackwardsSandkingEpicenter = thisEntity:FindAbilityByName( "sandking_epicenter" )
...
function SandstormThink()
	local  count =  RandomInt(0, 10)
       if  count >= 5 and BackwardsSandkingEpicenter and thisEntity:GetReadyAbility(BackwardsSandkingEpicenter) then   --50%几率触发地震
           return  CastBackwardsSandkingEpicenter() 
       else
...
function CastBackwardsSandkingEpicenter()
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
		AbilityIndex = BackwardsSandkingEpicenter:entindex()
	})
