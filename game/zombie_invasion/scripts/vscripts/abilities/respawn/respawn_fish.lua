...
function RespawnFish(keys)	
    ...
    print(chance_spawn)
     if name == "npc_classic_chicken" then
         if chance_spawn < 1 then
			 Timers:CreateTimer(respawn_time,function()	
		         local unit = CreateUnitByName("npc_stronger_chicken", position , true, nil, nil, team)
     ...
     elseif name == "npc_stronger_chicken" then 
     	 	 Timers:CreateTimer(respawn_time,function()	
		         local unit = CreateUnitByName("npc_classic_chicken", position , true, nil, nil, team)
		         unit:SetForwardVector(caster.fw)
	         end)
	 end
end
