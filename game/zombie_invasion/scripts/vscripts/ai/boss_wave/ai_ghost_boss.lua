function Spawn( entityKeyValues )
    ...
    hExorcismAbility = thisEntity:FindAbilityByName( "death_prophet_exorcism_boss_wave" )
    hSuckAbility = thisEntity:FindAbilityByName( "pugna_life_drain_wave_boss" )
...
function GhostThink()
    ...
    local waypoint = Entities:FindByName( nil, "last_boss") 		-- Записываем в переменную 'waypoint' координаты бокса d_waypoint19 
 	...
 	if  not hExorcismAbility:IsFullyCastable() then
	if   hSuckAbility:IsFullyCastable() then
		return  CastSuck( enemies[ 1 ] )
	end
    end
    
	if   hExorcismAbility:IsFullyCastable() then
		return  CastExr()
	end
...
function CastExr()
	ExecuteOrderFromTable({
		...
		AbilityIndex = hExorcismAbility:entindex(),
	})

	return 1.00
end

function CastSuck( enemy )
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_TARGET,
					TargetIndex = enemy:entindex(),
		AbilityIndex = hSuckAbility:entindex(),
	})
