function Spawn( entityKeyValues )
    ...
    hDism = thisEntity:FindAbilityByName( "pudge_dismember_wave" )  
    agro = false 
...
function ZombieThink()
	...
	if #enemies == 0 then
        ...
        if hDism:IsFullyCastable() then 
            CastDism( enemy )
        end     	if agro == false then 
...
function CastDism( enemy )
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_TARGET,
		TargetIndex = enemy:entindex(),
		AbilityIndex = hDism:entindex(),
	})
