if item_bewstheaks == nil then
	item_bewstheaks = class({})
...
function item_bewstheaks:CastFilterResultTarget(hTarget)
	--print("Error")
	if IsServer() then
...
function item_bewstheaks:GetCustomCastErrorTarget(hTarget)
	--print("Error")
	if IsServer() then
		if not hTarget:IsRealHero() then
...
function item_bewstheaks:OnSpellStart()
	--print("OnSpellStart")
