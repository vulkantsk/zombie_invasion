...
function RespawnPig(keys)	
    ...
    print(chance_spawn)
     if name == "npc_classic_pig" then
         if chance_spawn < 1 then
			 Timers:CreateTimer(respawn_time,function()	
		         local unit = CreateUnitByName("npc_classic_big_pig", position , true, nil, nil, team)
     ...
     elseif name == "npc_classic_big_pig" then 
     	 	 Timers:CreateTimer(respawn_time,function()	
		         local unit = CreateUnitByName("npc_classic_pig", position , true, nil, nil, team)
		         unit:SetForwardVector(caster.fw)
	         end)
	 end
end
