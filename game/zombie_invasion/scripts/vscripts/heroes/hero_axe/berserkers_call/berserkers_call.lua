...
function ability_berserkers_call:OnSpellStart()
	...
	local ability                   =       self
    local duration_enemy = ability:GetSpecialValueFor("duration")
    local duration                        
    if self:GetCaster():HasShard() then 
       duration =   ability:GetSpecialValueFor("shard_duration")  
    else 
	...
	for _,target in pairs(enemies_in_radius) do
		...
		target:AddNewModifier(caster, self, "modifier_ability_berserkers_call_debuff_cmd", {duration = duration_enemy * (1 - target:GetStatusResistance())})
	end
	...
	local armor_modif = caster:AddNewModifier(caster, self, "modifier_ability_berserkers_call_buff_armor", {duration = duration})
	if self:GetCaster():HasShard() then 
	armor_modif:SetStackCount(#enemies_in_radius)
	 end 
...
function modifier_ability_berserkers_call_buff_armor:GetModifierPhysicalArmorBonus()
	if self:GetParent():HasModifier("modifier_item_aghanims_shard") then 
	    return self:GetAbility():GetSpecialValueFor("bonus_armor") + ( self:GetAbility():GetSpecialValueFor("shard_armor") * self:GetStackCount() )
	   else 
