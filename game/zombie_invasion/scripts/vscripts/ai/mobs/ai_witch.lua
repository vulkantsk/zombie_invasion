function Spawn( entityKeyValues )
	...
	thisEntity:SetContextThink( "witchThink", witchThink, 1 )
end
...
function witchThink()
    ...
    local spawners = Entities:FindAllByName("npc_classic_witch_point")
...
function MoveTo(point)
    ...
    if move_count%RandomInt(5,9) == 1 then 
	    ExecuteOrderFromTable({
		    UnitIndex = thisEntity:entindex(),
		    OrderType = DOTA_UNIT_ORDER_MOVE_TO_POSITION,
		    Position = point:GetOrigin() +RandomVector(RandomInt(75,90))
	    })
	    return 1
	end
	    return 1
