LinkLuaModifier("modifier_marci_fragment_of_logic", "heroes/hero_marci/fragment_of_logic.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_marci_fragment_of_logic_debuff", "heroes/hero_marci/fragment_of_logic.lua", LUA_MODIFIER_MOTION_NONE)

sara_fragment_of_logic = class({
	GetIntrinsicModifierName = function() return "modifier_marci_fragment_of_logic" end,
})

if IsClient() then
	function marci_fragment_of_logic:GetManaCost()
		return self:GetSpecialValueFor("energy_const") + self:GetCaster():GetMaxMana() * self:GetSpecialValueFor("energy_pct") * 0.01
	end
end


modifier_marci_fragment_of_logic = class({
	IsHidden   = function() return true end,
	IsPurgable = function() return false end,
})

if IsServer() then
	function modifier_marci_fragment_of_logic:DeclareFunctions()
		return {
	...
	function modifier_marci_fragment_of_logic:OnTakeDamage(keys)
		...
		if parent == self:GetParent() and parent:GetHealth() <= 1 and not parent:IsIllusion() and parent.GetEnergy then
			...
			if ability:IsCooldownReady() and parent:GetEnergy() >= toWaste then
				...
				parent:AddNewModifier(parent, ability, "modifier_marci_fragment_of_logic_debuff", {duration = ability:GetSpecialValueFor("debuff_duration")})
	...
	function modifier_marci_fragment_of_logic:GetMinHealth(keys)
...
modifier_marci_fragment_of_logic_debuff = class({
	IsPurgable = function() return false end,
