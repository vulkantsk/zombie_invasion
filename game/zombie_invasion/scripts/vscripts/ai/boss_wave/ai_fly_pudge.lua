function Spawn( entityKeyValues )
	...
	if IsServer() == false then
 	...
 	hSpawner = Entities:FindByName( nil, "final_point" )
      local waypoint = Entities:FindByName( nil, "ghost_spawner") 		-- Записываем в переменную 'waypoint' координаты бокса d_waypoint19
    ...
    hHookAbility = thisEntity:FindAbilityByName( "pudge_meat_hook_fly" )
...
function PudgeMiniThink()
	...
	local npc = thisEntity
 	...
 	local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 1450, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC , DOTA_UNIT_TARGET_FLAG_NOT_ANCIENTS, FIND_CLOSEST, false )
