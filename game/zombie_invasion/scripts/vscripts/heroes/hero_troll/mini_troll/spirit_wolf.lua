LinkLuaModifier("modifier_spirit_wolf", "heroes/hero_troll/mini_troll/spirit_wolf", LUA_MODIFIER_MOTION_NONE)
...
function lycan_spirit_wolf_custom:OnSpellStart()
	...
	if  caster.wolf and IsValidEntity(caster.wolf) and caster.wolf:IsAlive() then
		...
		caster.wolf:SetBaseDamageMin(base_dmg + agility*dmg_per_strenght )
		caster.wolf:SetBaseDamageMax(base_dmg + agility*dmg_per_strenght )				
...
--]]
		...
		SetLevelForSubAbility(ability, "spectre_dispersion_troll", caster.wolf, 1, 1)
		SetLevelForSubAbility(ability, "legion_commander_moment_of_courage_troll", caster.wolf, 1, nil)
